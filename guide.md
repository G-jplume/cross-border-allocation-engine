...
- SKU级集中度自动检测：系统自动算每个SKU在目标期同月的出单量占全年的比例。集中度≥80%→自动强季节（即使品类未勾选），≤20%→自动弱季节（即使品类已勾选），中间→跟随品类勾选
...
计算完成后展示：
- 自动体检：四仓占比合计=100%、季节品类分支生效、趋势因子生效数（含动态α）
- KPI卡片：美西、美东、美南GA、美南TX 四仓平均占比（含与基准的差值）
...
- 实时显示四仓合计是否=100%
- 点击「自动补足到 100%」，以改动最大的仓为准，其余三仓按原比例自动补足
- 点击「🔄 恢复引擎值」可一键回到系统计算结果
...
- 堆叠图：支持按 SKU 名搜索，分页浏览全部SKU，合计恒为 100%
