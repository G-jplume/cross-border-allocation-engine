...
- SKU级集中度自动检测：两个滑块（整数范围，高阈值50-100%默认80%，低阈值0-50%默认20%）。系统自动算每个SKU在目标期同月的出单量占全年的比例。集中度≥高阈值→自动强季节（即使品类未勾选），≤低阈值→自动弱季节（即使品类已勾选），中间→跟随品类勾选
...
- 集中度≥80% → 自动强季节（即使品类未勾选）：只用目标期同月数据
- 集中度≤20% → 自动弱季节（即使品类已勾选）：用全部月份加权
- 中间（20%-80%）→ 跟随品类勾选：勾选的品类用同月数据，未勾选的用全部月份
...
新品（历史月数≤2 或 目标期原始单量<10）自身权重=0，完全使用基准。

趋势调整用原始单量占比，比较去年同期 vs 前年同期的占比差。单量门槛+动态α：去年+前年同期总单量≥100用原始α，30-100单用α/2，<30单或任一侧为0单跳过趋势调整。
...
### Q2：强季节和弱季节品类怎么区分？
三步判断：①先算SKU集中度（目标期同月出单量÷全年总量）→②集中度≥高阈值（默认80%）自动强季节，≤低阈值（默认20%）自动弱季节→③集中度在中间的跟随品类勾选。强季节只用目标期同月数据，弱季节用全部月份加权。阈值可在"开始计算"区域滑块调节。
