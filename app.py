...
分仓占比计算引擎用于计算各 SKU 在四个仓库（美西、美东、美南GA、美南TX）的发货占比。
...
**侧边栏（参数面板）**：① 目标发货月份 → ② 时间衰减加权 → ③ 季节匹配因子（默认折叠）→ ④ 趋势因子（默认折叠）→ ⑤ 新品与基准（默认折叠）

**主区域（操作区）**：1.上传数据 → 2.开始计算（蓝色边框容器）→ 3.计算结果 → 4.占比微调 → 5.可视化 → 6.导出结果
...
选择起始年月和结束年月（如 2026年12月 → 2027年3月），支持跨年。锚点 = 目标期中间月份。强季节品类只看目标期同月数据，弱季节品类放开全部月份。
...
分类逻辑：强季节品类只看目标期同月数据，用 λ_same 衰减；弱季节品类同月用 λ_same，非同月用 λ。

### 3.3 季节匹配因子

| 参数 | 默认值 | 说明 |
|---|---|---|
| 强季节增强倍数 β | 3.0 | 强季节品类同季数据权重放大倍数 |
| 弱季节增强倍数 β_weak | 1.5 | 弱季节品类同月数据权重放大倍数。1.0=不放大 |
| 季节窗口范围 N | 1 | 窗口 = 目标月前 N 个月（环形距离） |

适用品类：从上传数据中动态识别，默认勾选「庭院、草坪与花园」和「庭院」。
...
趋势因子比较去年 vs 前年同期的原始单量占比差，使用原始数据计算，不受衰减和季节因子干扰。

### 3.5 新品与基准

| 参数 | 默认值 | 说明 |
|---|---|---|
| 新品阈值（月） | 2 | 历史出单月数 ≤ 此值的 SKU 视为新品 |
| 新品订单量门槛 | 10 | 目标期总单量 < 此值也视为新品 |
| 收缩强度 k | 6 | 自身权重 = n/(n+k)，k越大越信任基准 |
| 权重下限 | 0.0 | 自身数据的最低权重 |
| 权重上限 | 0.9 | 自身数据的最高权重 |
| 品类层最小等效观测数 | 12 | 贝叶斯收缩参数。n=12时子层父层各50% |
...
### 4.1 上传数据
必需列：源SKU、SPU、室内外、一级分类、年、月、美西、美东、美南GA、美南TX。

### 4.2 开始计算
蓝色边框容器内设置季节适用品类多选、减仓优化开关，点击「开始计算」后系统分7步逐步显示进度。

### 4.3 计算结果
自动体检、KPI卡片（四仓平均占比）、SKU级表格（5个分页Tab）、聚合层级占比。

### 4.4 占比微调
拖动滑块调整四仓占比，自动补足到100%，输入发运批量生成落货量（最大余额法）。

### 4.5 可视化
饼图（四仓占比+标签）、可搜索堆叠图（按SKU名筛选，每页10-100）。

### 4.6 导出结果
Excel（含聚合）、CSV（SKU级）、CSV（一级分类）。

---

## 5. 计算逻辑说明（7步）

1. **运算SKU映射**：混用SKU映射表合并；未映射的取「-」前部分
2. **时间衰减加权**：强季节只看同月用 λ_same(0.95)；弱季节同月用 λ_same，非同月用 λ(0.85)
3. **季节匹配因子**：强季节+窗口内 → 权重×β(3.0)；弱季节+同月 → ×β_weak(1.5)；其他 ×1.0
4. **自身占比**：加权后各仓出单量 ÷ 加权合计
5. **基准占比**：层级回退 SPU→一级分类→室内外→全公司，每层贝叶斯收缩
6. **最终占比**：自身×自身占比 + (1-自身)×基准，叠加趋势调整，归一化到100%
7. **落货量**：占比 × 批量，最大余额法保证整数合计精确

减仓优化（可选）：月均<3单且占比<5%的仓位减仓，等比分配到其他仓。仅影响SKU层，上层聚合用减仓前原始数据。
...
趋势因子比较去年同月 vs 前年同月的原始单量占比差。多月目标期则参考对应月份范围。

**Q2：强季节和弱季节品类怎么区分？**
强季节品类 = 侧边栏「季节适用品类」多选框中勾选的品类（默认庭院类）。不在列表中的自动归为弱季节。

**Q3：同月跨年衰减率 λ_same 和非同月衰减率 λ 有什么区别？**
λ_same(0.95) 用于同月数据跨年衰减，去年同月保留95%。λ(0.85) 用于弱季节品类的非同月数据。同月数据参考价值更高，所以衰减更慢。

**Q4：新品怎么判定？**
历史出单月数 ≤ 2 或 目标期总单量 < 10，满足任一即视为新品。新品自身权重=0，完全使用基准占比。

**Q5：减仓优化影响上层聚合吗？**
不影响。减仓仅修改SKU层，一级分类/室内外/SPU/全公司的聚合使用减仓前原始数据。
