...
| 趋势单量门槛 | 30 | 去年+前年同期总单量低于此值跳过趋势调整 |
...
### 步骤2：时间衰减加权
时间衰减 × 月份相似度。同月数据用 λ_same（0.95）跨年衰减。非同月数据用 λ（0.85）衰减，再乘月份相似度权重（同月1.0/相邻月0.6/隔2月0.3/隔3月以上0.1），让目标月附近的非同月数据也有参考价值。
...
趋势调整用原始单量占比，比较去年同期 vs 前年同期的占比差。单量门槛+动态α：去年+前年同期总单量≥100用原始α，30-100单用α/2，<30单跳过趋势调整。
...
趋势因子用原始单量占比（不加衰减权重），比较去年同期 vs 前年同期的分仓占比差。动态α：≥100单用原始α，30-100单用α/2，<30单跳过趋势调整。与时间衰减加权作用层不同，互不干扰。
"""
