...
with st.expander("为什么需要这个工具？", expanded=False):
    st.markdown(
        "**公司现状**：运算发货月份的分仓数据时，仅参考去年同期的订单数据。\n\n"
        "这一做法存在三个问题：\n\n"
        "| 问题 | 说明 |\n"
        "|---|---|\n"
        "| 非季节性产品缺乏合理性 | 非季节性产品分仓稳定，仅用去年同月数据样本量小、偶然性大 |\n"
        "| 新品数据不稳定 | 新品上架初期出单量小、分仓随机性大，直接照搬去年同期会失真 |\n"
        "| 其他年份数据未合理利用 | 前年同月、其他月份数据被完全忽略，大量历史数据闲置 |\n\n"
        "**本工具的解决方案**：\n\n"
        "| 问题 | 对应方案 |\n"
        "|---|---|\n"
        "| 非季节性产品 | 使用全部历史月份数据 + 时间衰减加权（越近权重越大）+ 月份相似度 |\n"
        "| 新品不稳定 | 自身权重为0，完全使用基准占比（SPU→品类→室内外→全公司层层回退+贝叶斯收缩）|\n"
        "| 其他年份未利用 | 双层加权：时间衰减（同月跨年λ_same + 非同月λ）+ 趋势因子（去年vs前年同期占比差）|"
    )
